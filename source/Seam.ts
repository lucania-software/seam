...
export class Seam {
    ...
    public async registerPlugins(...names: string[]) {
        for (const name of names) {
            let resolution;
            try {
                resolution = import.meta.resolve(name, pathToFileURL(this._getUsagePackageJson()));
            } catch (error) {
                Data.assert(error instanceof Error, "Threw an error that wasn't of Error type.");
                console.error(`Failed to resolve entry point of ${name}. ${error.message}`);
            }
    ...
    public async installPlugins(...specifiers: string[]) {
        ...
        specifiers = this._getNormalizedPluginSpecifiers(specifiers);
        try {
            ...
            packageJson = await this._readPackageJson();
            this._cacheNumber++;
            // Created cached copy of installed node modules based on _cacheRefreshCounter.
            if (this._pluginsCacheDirectory !== undefined && this._cacheNumber - 1 > 0) {
    ...
    private _getNormalizedPluginSpecifiers(pluginSpecifiers: string[]) {
        ...
        for (const pluginSpecifier of pluginSpecifiers) {
            if (pluginSpecifier.startsWith(fileSpecifierPrefix)) {
                ...
                const filePath = resolve(this._rootDirectory, pluginSpecifier);
                const relativeFilePath = relative(this._pluginsInstallationDirectory, filePath);
                const npmRelativeFilePath = posix.join(...relativeFilePath.split(separator));
                normalizedPluginSpecifiers.push(fileSpecifierPrefix + npmRelativeFilePath);
            } else {
    ...
    private _getUsageDirectory() {
        const currentCacheNumber = this._cacheNumber - 1;
        if (this._pluginsCacheDirectory === undefined || currentCacheNumber === 0) {
            return this._pluginsInstallationDirectory;
        } else {
            return join(this._pluginsCacheDirectory, currentCacheNumber.toString());
        }
    }
